...
router.post("/assets/upload-url", async (req: Request, res: Response): Promise<void> => {
  ...
  const uploadUrl = `${origin}/api/assets/upload?key=${encodeURIComponent(key)}`;
  const fileUrl = `${origin}/uploads/${key}`;
...
router.put("/assets/upload", async (req: Request, res: Response): Promise<void> => {
  const key = typeof req.query.key === "string" ? req.query.key : undefined;
  if (!key) {
    res.status(400).json({ error: "Missing upload key" });
    return;
  }

  const uploadPath = path.join(uploadsBase, key);
