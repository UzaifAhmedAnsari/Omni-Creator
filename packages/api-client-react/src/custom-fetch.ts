...
let _baseUrl: string | null = getDefaultBaseUrl();
...
function normalizeRelativePathForBase(path: string, baseUrl: string): string {
  if (baseUrl.endsWith("/api") && path === "/api") {
    return "";
  }
...
function applyBaseUrl(input: RequestInfo | URL): RequestInfo | URL {
  ...
  const normalizedPath = normalizeRelativePathForBase(url, _baseUrl);
